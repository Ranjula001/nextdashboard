...
/**
 * Get all active rooms for the authenticated owner
 */
...
/**
 * Get rooms available for booking in a date range
 */
export async function getAvailableRoomsForDateRange(
  ...
  if (error) {
    ...
    // Return all active rooms if there's an error
    return getRooms();
  }

  // Get all rooms and filter out booked ones
